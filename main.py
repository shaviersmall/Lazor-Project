...
class Solver:
    def __init__(self, game_board):
       ...
       self.hit_points = game_board.hit_points
    ...
    def solve(self):
        for block_config in self.move_blocks():
            ...
            for laser_start, direction in self.laser_pts:
                self.hit_points.update(self.move_laser(laser_start, direction, board))
            if all(target in self.hit_points for target in self.soln_pts):
    ...
    def check_solution(self): #if laser_pos == soln_pos
        return all(point in self.hit_points for point in self.grid.soln_pts)
